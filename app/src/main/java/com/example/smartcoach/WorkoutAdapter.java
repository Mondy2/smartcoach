...
public class WorkoutAdapter extends RecyclerView.Adapter<WorkoutAdapter.HomeWorkoutViewHolder> {
    ...
    public void onBindViewHolder(@NonNull HomeWorkoutViewHolder holder, int position) {
        ...
        // 1) Сырая группа (англ.) с фолбэком
        String rawGroup = workout.getMuscleGroup();
        if (rawGroup == null || rawGroup.isEmpty()) {
            rawGroup = WorkoutUtils.determineMuscleGroup(workout.getSuggestedWorkout());
        }

        // 2) Переводим на UI-язык (например, "Груди")
        String uiGroup = WorkoutUtils.translateMuscleGroup(rawGroup);

        // 3) Получаем ключ ("chest", "back" и т.д.)
        String keyGroup = WorkoutUtils.uiToKeyMuscleGroup(uiGroup);

        // 4) Ставим картинку по ключу
        WorkoutUtils.setMuscleImage(holder.muscleImage, keyGroup);
